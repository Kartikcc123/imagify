  ...
  return (
    <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      ...
      <form className='relative bg-white p-10 rounded-xl text-slate-500'> 
        ...
        <button className='bg-blue-600 w-full text-white pu-2 rounded-full'>{state === "Login" ? "login" : "create account"}</button>
       ...
       {state === "Login" ? <p className='mt-5 text-center'>Don't have an account? 
        ...
        :
        <p className='mt-5 text-center' onClick={()=>setState('Login')}>Already have an account? 
